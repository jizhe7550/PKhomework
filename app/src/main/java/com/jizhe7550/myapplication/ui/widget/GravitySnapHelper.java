...
public class GravitySnapHelper extends LinearSnapHelper {
    ...
    public interface ItemViewAnimListener {
        /**
         * handleAnim when scroll or idle
         *
         * @param recyclerView
         * @param position
         * @param newState
         */
        void onItemViewAnim(RecyclerView recyclerView, int position, int newState);
    }
    ...
    private ItemViewAnimListener animListener;
    ...
    private RecyclerView.OnScrollListener scrollListener = new RecyclerView.OnScrollListener() {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (animListener != null) {
                animListener.onItemViewAnim(recyclerView, nextSnapPosition, newState);
            }
        }
    };

    public GravitySnapHelper(SnapGravity gravity) {
        this(gravity, false, null);
    }

    public GravitySnapHelper(SnapGravity gravity, @NonNull SnapListener snapListener) {
        this(gravity, false, snapListener);
    }

    public GravitySnapHelper(SnapGravity gravity, boolean enableSnapLastItem) {
...
//        }
    ...
    public View findSnapView(@NonNull RecyclerView.LayoutManager lm, boolean checkEdgeOfList) {
        ...
        switch (gravity) {
            case START:
                snapView = findView(lm, getHorizontalHelper(lm), START, checkEdgeOfList);
                break;
            case END:
                snapView = findView(lm, getHorizontalHelper(lm), END, checkEdgeOfList);
                break;
            case TOP:
                snapView = findView(lm, getVerticalHelper(lm), START, checkEdgeOfList);
                break;
            case BOTTOM:
                snapView = findView(lm, getVerticalHelper(lm), END, checkEdgeOfList);
                break;
            case CENTER:
                if (lm.canScrollHorizontally()) {
                    snapView = findView(lm, getHorizontalHelper(lm), CENTER,
                            checkEdgeOfList);
                } else {
                    snapView = findView(lm, getVerticalHelper(lm), CENTER,
                            checkEdgeOfList);
                }
                break;
        }
    ...
    public int[] calculateDistanceToFinalSnap(@NonNull RecyclerView.LayoutManager layoutManager,
        ...
        if (lm.canScrollHorizontally()) {
            if ((isRtl && gravity == END) || (!isRtl && gravity == START)) {
    ...
    public void setItemViewAnimListener(@Nullable ItemViewAnimListener listener) {
     ...
     */
    public void setGravity(SnapGravity newGravity) {
     ...
     */
    public void setGravity(SnapGravity newGravity, Boolean smooth) {
    ...
    private View findView(@NonNull RecyclerView.LayoutManager layoutManager,
        ...
        final boolean snapToStart = (gravity == START && !isRtl)
                || (gravity == END && isRtl);

        final boolean snapToEnd = (gravity == START && isRtl)
                || (gravity == END && !isRtl);
    ...
    private boolean isAtEdgeOfList(LinearLayoutManager lm) {
        if ((!lm.getReverseLayout() && gravity == START)
                || (lm.getReverseLayout() && gravity == END)
                || (!lm.getReverseLayout() && gravity == TOP)
                || (lm.getReverseLayout() && gravity == BOTTOM)) {
